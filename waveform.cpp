...
const int Waveform::SAMPLE_DURATION = 1;   //采样间隔，原始30，单位应为毫秒
const int Waveform::WAVE_WIDTH = 1;         //线条宽度，原始2
const int Waveform::WAVE_DURATION = 1;      //线条间宽，原始4
...
void Waveform::paintEvent(QPaintEvent *)
{
    ...
    for (int i = 0; i < sampleList.size(); i++) {
        volume = sampleList[i] * rect().height(); //原始volume = sampleList[i] * rect().height() * 2;

        if (volume == 0) {
            QPainterPath path;
            path.addRect(QRectF(rect().x() + i * WAVE_DURATION, rect().y() + (rect().height() - 1) / 2, WAVE_DURATION, 1));
            painter.fillPath(path, QColor("#FFFFFF"));  //原#FFA0A0
        } else {
            ...
            gradient.setColorAt(0, QColor("#2CA7F8"));  //原#FFBD78
            gradient.setColorAt(1, QColor("#B2DCF8"));  //原#FF005C
    ...
    if (sampleList.size() < rect().width() / WAVE_DURATION) {
        ...
        path.addRect(QRectF(rect().x() + sampleList.size() * WAVE_DURATION,
                            ...
                            1));
        painter.fillPath(path, QColor("#6FC1F7"));  //原#FFA0A0
    }
}
